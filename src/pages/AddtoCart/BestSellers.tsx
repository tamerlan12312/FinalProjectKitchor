...
const BestSellers = () => {
  const ctx = useContext(LangContext)
   ...
   const handleActive =(btn: React.SetStateAction<number>)=> setIsActive(btn)
    const {url} = useParams()
    const bestDetailsItem:any = productsData.find(p=>p.id == url)
    useEffect(()=>{
...
<Helmet>
  ...
  <section className="bestDetails">
<div className="container-fluid">
<div className="d-flex">
    <Link to="/" className='besta'><h6 className="besth6" title="Homepage">{ctx.lang.virtualtours.home}</h6></Link>
    <h6 className="ms-2 me-2 besth62">/</h6>
    <h6 className="besth6" title={`${bestDetailsItem.title2}`}>{bestDetailsItem.title2}</h6>
    <h6 className="ms-2 me-2 besth62">/</h6>
    <h6 className="besth6" title={`${bestDetailsItem.title}`}>{bestDetailsItem.title}</h6>
</div>
<div className="row my-4">
    <div className="col-12 col-sm-12 col-md-5 col-lg-5 ">
        <div className="bestDetailsImg">
            <img src={bestDetailsItem.img2} alt="" className="img-fluid animate__animated animate__fadeIn"  />
        </div>
    </div>
    ...
    <div className="detailsStock animate__animated animate__fadeIn">
        <h6 className="bestdetailsh6 ">{ctx.lang.shop.bestsellers1}</h6>
        </div>
    </div>
    </div>
    <h1 className="detailsh1 mt-2 animate__animated animate__fadeIn">{bestDetailsItem.title}</h1>
   <div className="row">
    ...
    <span className='stockSpan'>{bestDetailsItem.title3}</span>
    </div>
   </div>
    <hr className="hr" />
    <h4 className='detailsPriceh4'>${bestDetailsItem.price}</h4>
    <p className='detailsP2'>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Odio quisquam est alias sed cumque perspiciatis quaerat facere odit corrupti accusantium a quis aut fugiat tempore nam expedita optio quasi nobis rerum iste necessitatibus aperiam, consequatur voluptatem? Voluptas aliquid quod enim animi veritatis, sint, eveniet labore ab harum incidunt ipsam beatae, nemo dolor suscipit quis ut? Repudiandae maiores ad quaerat exercitationem?</p>
      ...
      <button className='stockBtn'  onClick={()=>{
        toast.info(ctx.lang.toastify.addtocart2, {
          position: "top-center",
          autoClose: 1000,
      ...
      <div className="detailsIconTxt">
      <i className="fa-regular fa-heart"></i>
      <span className='ms-2'>{ctx.lang.shop.bestsellers3}</span>
      </div>
